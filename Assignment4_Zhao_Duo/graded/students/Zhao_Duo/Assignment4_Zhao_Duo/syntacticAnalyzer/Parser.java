...
public class Parser {
	...
	void accept(Type tokenExpected){ // conditionally accept
		if(currentToken.type == tokenExpected){
			...
			System.out.println(
					...
					+ " encountered");
            // GRADING: Do not use System.exit() outside of main().  Instead,
            // use exceptions
			System.exit(4);
		}
	}
	...
	public void parseStmt(){
		if (currentToken.type == Token.Type.READ){// stmt --> read id
			...
			parseId();
            // GRADING: Multiple levels of "if" statements usually indicates
            // not LL(1), but I'm assuming you're just using a non-BNF grammar
